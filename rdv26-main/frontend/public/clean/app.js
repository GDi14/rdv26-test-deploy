...
function getThemeColors() {
  ...
  const fallbackText = isDark ? '#D4E5FB' : '#1C1C2E';
...
class GlobeGallery {
  ...
  createZoomOverlay() {
    ...
    overlay.innerHTML = `
      ...
      <div class="zoom-overlay-content">
        <button type="button" class="zoom-overlay-close" aria-label="Close enlarged image">×</button>
        <img class="zoom-overlay-img" alt="Enlarged gallery image" />
      </div>
    `;
    overlay.addEventListener('click', (e) => {
      if (e.target === overlay || e.target.classList.contains('zoom-overlay-backdrop')) {
    ...
    this.zoomedImage = overlay.querySelector('.zoom-overlay-img');
    const closeBtn = overlay.querySelector('.zoom-overlay-close');
    if (closeBtn) {
      closeBtn.addEventListener('click', () => this.hideZoomOverlay());
    }
  }

  showZoomOverlay(src) {
    if (!this.zoomOverlay || !this.zoomedImage || !src) return;
    this.zoomedImage.src = src;
    this.zoomOverlay.classList.add('open');
    this.zoomOverlayVisible = true;
  }

  hideZoomOverlay() {
    if (!this.zoomOverlay) return;
    this.zoomOverlay.classList.remove('open');
    this.zoomOverlayVisible = false;
  }

  handleGridClick(px, py) {
    if (!this.galleryState || this.galleryState !== 'grid') return;
    ...
    const tileData = this.cmsData[idx % this.cmsData.length];
    if (!tileData || tileData.mediaType !== 'image') return;
    this.showZoomOverlay(tileData.mediaSrc);
  }
  ...
  onMouseUp = (e) => {
    ...
    const wasClick = !this.isDragging && elapsed < this.clickThreshold.time && this.mouseMoveDistance <= this.clickThreshold.distance;
    ...
    if (wasClick && this.galleryState === 'grid') {
  ...
  onKeyDown = (e) => {
    if (e.key === 'Escape' && this.zoomOverlayVisible) {
      e.preventDefault();
      this.hideZoomOverlay();
      return;
    }
